...
Place the code for this question in a file called
\texttt{Frequency.java} and copy any output in your PDF to answer this
question.
...
Dijkstra's\footnote{Note that Java's class
  \href{https://docs.oracle.com/javase/8/docs/api/java/util/PriorityQueue.html}{\texttt{PriorityQueue}}
  is not \href{https://stackoverflow.com/q/2288241/12035739}{dynamic}
  and breaks ties arbitrarily.} and the Bellman-Ford algorithms. Use
your Dijkstra's implementation to solve the files
\texttt{dijkstra1.txt} and \texttt{dijkstra2.txt}. Similarly, use your
Bellman-Ford algorithm to solve the files \texttt{bellmanford1.txt}
and \texttt{bellmanford2.txt}. For each graph and algorithm, report
the minimum distance with which each vertex can be reached and their
respective parent vertex in your answers PDF. E. g, the output of
Dijkstra's algorithm on \texttt{dijkstra1.txt} should look like,
...
\begin{enumerate}
    \item Turn in a PDF containing any outputs and answers from the homework.
    \item Also turn in your \texttt{Frequency.java},
    \texttt{Base27.java} (with a main method) and finally the
    \texttt{Graph.java}.
\end{enumerate}
